...
#create dataset
datasetTrain = DatasetMRI2D(root_dir_img=Path(config.dataset_train_path), root_dir_segm=Path(config.segm_train_path), connected_masks=config.train_only_connected_masks, target_shape =config.t1n_target_shape, sorted_slice_sample_size=config.num_sorted_samples, transforms=transformations, random_sorted_samples=True)
datasetEvaluation = DatasetMRI2D(root_dir_img=Path(config.dataset_eval_path), root_dir_masks=Path(config.masks_eval_path), connected_masks=config.eval_only_connected_masks, target_shape =config.t1n_target_shape, sorted_slice_sample_size=config.num_sorted_samples)
