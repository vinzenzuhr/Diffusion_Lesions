...
class TrainingConfig:
    ...
    save_model_epochs = 450
    mixed_precision = "fp16"  # `no` for float32, `fp16` for automatic mixed precision
    output_dir = "lesion-filling-256_uncond"  # the model name locally and on the HF Hub
    ...
    pretrained_path = "./lesion-filling-256_uncond/pretrained"
    num_gpu=2
    ...
    for epoch in range(config.num_epochs): 
        ...
        if accelerator.is_main_process:
            ...
            if (epoch) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1: 
                ...
                accelerator.unwrap_model(model).save_pretrained(config.pretrained_path)
