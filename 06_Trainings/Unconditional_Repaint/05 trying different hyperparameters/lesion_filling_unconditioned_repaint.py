...
def evaluate(config, pipeline, eval_dataloader, prefix=""):
    ...
    images = pipeline(voided_images, masks, generator=torch.manual_seed(config.seed), eta=1.).images
    ...
    image_grid.save(f"{test_dir}/{prefix}_eta_inpainted_image.png")
    
    pil_voided_images = [to_pil_image(x) for x in voided_images]
    voided_image_grid = make_image_grid(pil_voided_images, rows=2, cols=2)
    voided_image_grid.save(f"{test_dir}/{prefix}_eta_voided_image.png")

    ####

    images = pipeline(voided_images, masks, generator=torch.manual_seed(config.seed), jump_length=2).images

    # Make a grid out of the images
    image_grid = make_image_grid(images, rows=2, cols=2)

    # Save the images 
    image_grid.save(f"{test_dir}/{prefix}_jumpLength_inpainted_image.png") 

    #

    images = pipeline(voided_images, masks, generator=torch.manual_seed(config.seed), jump_n_sample =2).images

    # Make a grid out of the images
    image_grid = make_image_grid(images, rows=2, cols=2)

    # Save the images 
    image_grid.save(f"{test_dir}/{prefix}_jumpSamples_inpainted_image.png") 

    #

    images = pipeline(voided_images, masks, generator=torch.manual_seed(config.seed), jump_n_sample =2, jump_length=2).images
...
pipeline = RePaintPipeline(unet=model, scheduler=RePaintScheduler(eta=1.))
evaluate(config, pipeline, eval_dataloader, prefix="schedulerEta_")
