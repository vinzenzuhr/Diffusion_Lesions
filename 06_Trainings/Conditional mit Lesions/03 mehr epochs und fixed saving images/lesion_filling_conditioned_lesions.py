...
def evaluate(config, epoch, pipeline, eval_dataloader, global_step, tb_summary):
    ...
    print("SSIM_global: ", SSIM)
    print("PSNR_global: ", PSNR)
    ...
    clean_image_grid = make_image_grid(pil_clean_images, rows=int(config.eval_batch_size**0.5), cols=int(config.eval_batch_size**0.5))
...
def dim3evaluate(config, epoch, pipeline, dim3eval_dataloader, global_step, tb_summary):
    ...
    clean_images = batch["gt_image"][0] #torch.Size([1, 1, 256, 256, 256])
    masks = batch["mask"][0]  #torch.Size([1, 1, 256, 256, 256])
    max_v = batch["max_v"][0]
    idx = batch["idx"][0]

    #jedes sample einzeln. Shape: [1, 256, 256, 256]

    #debug
    masks2 = torch.ones_like(masks)
    masks2[:, 140:150,140:143,140:150] = 0
    masks = masks2
    ...
    stacked_void_mask = stacked_void_mask.permute(0, 1, 3, 2, 4) 
    chunks = torch.chunk(stacked_void_mask, math.ceil(stacked_void_mask.shape[2]/config.eval_batch_size), dim=2)
    ...
    datasetEvaluation.save(clean_images, "./new.nii.gz", **datasetEvaluation.get_metadata(int(idx)))
